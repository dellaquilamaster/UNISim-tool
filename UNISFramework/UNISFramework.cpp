...
void UNISFramework::PrintConfiguration() const
{
  ...
  printf("----------------------\n");
  printf("Data Source: %s\n", fGenerateData ? "Generated" : Form("From input file %s", fInputFileName.c_str()));
...
void UNISFramework::InitTree()
{
  ...
  if(!fGenerateData) {
    ...
    //
    //Reading tree
    fInputTree = (TTree*)fInputFile->Get("t");
    //

    if(fInputFile->IsZombie() || fInputTree==0) {
      printf("InitTree> Cannot initialize data source from file %s\nData will be generated by UNISim!\n", fInputFileName.c_str());
      fGenerateData=false;
      return;
    }
    //
    ...
    fTheReader=new TTreeReader(fInputTree);
    fTheInputEvent=new TTreeReaderValue<UNISRootEvent>(*fTheReader,Form("%s.", fExpSetup->GetName()));
    //
  }
  //
}
...
//____________________________________________________
void UNISFramework::RegisterEvent()
{
  //
  fTheReader->Next();
  //
  ...
  //
  UNISRootEvent * AnEvent = fTheInputEvent->Get();
  //
